...
lambda1 = 0.03 #arrival rate per sample
lambda2 = 0.03 #survival rate per sample
...
fig, ax = plt.subplots()
ax.plot(list(range(5*dLen*(upTimes[0]+downTimes[0]))),10*np.log10(totalAvgPwr[0:5*dLen*(upTimes[0]+downTimes[0])])-30)
ax.set(xlabel='Samples',ylabel='Total Average Power (dBm)')
fig, ax2 = plt.subplots()
ax2.plot(list(range(5*dLen*(upTimes[0]+downTimes[0]))),np.real(inputRF[0:5*dLen*(upTimes[0]+downTimes[0])]))
ax2.set(xlabel='Samples',ylabel='Input RF Signal')

plt.show()

with open("arp_dataset.csv", 'w') as arp_dataset:
