...
def predictor(self,numberOfSamples, datas):
    ...
    clf = svm.LinearSVR(epsilon=10e-12, C=10e12, max_iter=1000000, dual=True,random_state=0,verbose=6,loss='squared_epsilon_insensitive',tol=10e-11).fit(train_reshape,label_reshape)
...
def plotSVM(self, totalPwrLvl, prediction,sampleSize):
    ...
    plt.plot(np.array([i for i in np.arange(N_test-wLen)]),10*np.log10(abs(np.real(totalPwrLvl[0,N_train+wLen:sampleSize])))-30,np.array([i for i in np.arange(N_test-wLen)]),10*np.log10(np.abs(score))-30)
